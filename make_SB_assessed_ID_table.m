...
classMap_provider2WoRMS.data_provider_category_automated = {'Dinophyceae_morphotype_1' 'Dinophyceae_morphotype_2' 'Dinophyceae_morphotype3' 'Katodinium_or_Torodinium' 'Pyramimonas_morphotype1' 'Strombidium_morphotype1'...
    'ciliate' 'coccolithophorid' 'colony' 'crescent' 'pennate' 'pennate_morphotype1' 'square_unknown' 'unknown3_squiggle', 'Bacillariophyceae_morphotype1'}';
classMap_provider2WoRMS.data_provider_rename_automated = {'Dinophyceae' 'Dinophyceae' 'Dinophyceae' 'Dinophyceae' 'Pyramimonas' 'Strombidium'...
    'Ciliophora' 'Prymnesiophyceae' 'Biota' 'Biota' 'Bacillariophyceae' 'Bacillariophyceae' 'Bacillariophyceae' 'Dinophyceae' 'Bacillariophyceae'}';
classMap_provider2OtherNamespace.data_provider_category_automated = {'detritus' 'detritus_transparent' 'fecal_pellet' 'nanoplankton_mix' 'flagellate'}';
classMap_provider2OtherNamespace.data_provider_rename_automated = {'ptwg:detritus' 'whoi-plankton:detritus_transparent' 'ptwg:fecal_pellet' 'whoi-plankton:nanoplankton_mix' 'whoi-plankton:flagellate' }';
...
for ii = 1:size(assessed_ID_table,1)
    ...
    for ii = 1:size(assessed_ID_table,1)
        if assessed_ID_table.scientificNameID_automated(ii) % these are from WoRMS
                assessed_ID_table.scientificName_automated{ii} = [WoRMSstr webread(['https://www.marinespecies.org/rest/AphiaNameByAphiaID/' num2str(assessed_ID_table.scientificNameID_automated(ii))])];
            ...
            assessed_ID_table.scientificNameID_automated(ii) = missing; %non-WoRMS cases have no number
        end      
    end
    %%
notes = 'created with make_SB_assessed_ID_table.m';
save('C:\work\EXPORTS\assessed_ID_table', 'assessed_ID_table', 'notes')
writetable(assessed_ID_table, 'C:\work\EXPORTS\automated_assessed_id_EXPORTSNP_IFCB125.txt', 'WriteVariableNames', false, 'FileType', 'text')
